...
**4. 人体 / 手部骨骼精度无法定量评测**
无论 WHAM/SMPLify 出的身体骨骼还是 MediaPipe/MANO 出的手部骨骼，都没有**带真值的对照数据**（动捕标记 / 多视角重建），精度到底几个厘米、哪类动作会崩，目前只能目测回放，说不清。
→ 现状：只有内部一致性代理指标（重投影 p95、关键点可见率），不是真精度。
→ 缓解：用 ReViV / EgoExo 动捕链路（XREAL + 多视角）采一小批**双源对照集**当 ground truth，先覆盖典型动作原语，把"目测"升级为有数字的误差报告。

**5. Retarget 产物在 A5 / G1 上的有效性未完全闭环**
重定向结果"机器人能不能照着做"是最后一公里：**G1 已在 MuJoCo 完成验证，平衡性基本确认没问题**；但任务级有效性（手部接触、操作成功率）尚未验证，**A5 机械臂侧连仿真验证都还没做**（Placo IK 链路与 G1 不同，不能直接复用结论）。
